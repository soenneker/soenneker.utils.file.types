...
public sealed class FileTypeUtil : IFileTypeUtil
{
    ...
    public bool? TryGetContainerMediaSet(string extension, out MediaFormatSet? set)
    {
        if (!_containerMediaSets.TryGetValue(extension, out MediaFormatSet? mapped) || mapped is null)
        {
            set = null;
            return false;
        }

        set = new MediaFormatSet
        {
            AudioCodecs = [.. mapped.AudioCodecs],
            VideoCodecs = [.. mapped.VideoCodecs]
        };
